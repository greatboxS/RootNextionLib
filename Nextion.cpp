  ...
  while (m_serialPort.available() > 0)
  {
    ...
    if (c == NEX_RET_EVENT_TOUCH_HEAD)
    {
      ...
      if (m_serialPort.available() >= 6)
      {
        ...
        {
			root_touchableEvent->root_processEvent(buffer[1], buffer[2], buffer[3]);
...
void Nextion::root_registerTouchable(INextionTouchable* touchable)
{
	root_touchableEvent = touchable;
}
